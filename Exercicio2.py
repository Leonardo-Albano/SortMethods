...
def heuristica(posicaoAtual):
    posicaoFinal = 99
    valorHeuristica = 0
    valorHeuristica = abs(posicaoAtual%10 - posicaoFinal%10) + abs(posicaoAtual//10 - posicaoFinal//10)
    return valorHeuristica
#funçao busca A*
def buscaA(labirinto):
    ...
    labirintoTeste = labirinto
    caminhoTotal = 0
    heuristicaAtual = heuristica(posicaoAtual)
    while True:
        if(verificaPatoPreso(labirinto, posicaoAtual, heuristica)==1):
            break
        movimento = movimentaPato(labirinto, posicaoAtual,heuristica)
        if movimento == "d":
            caminhoTotal += movimentaPato(labirintoTeste, posicaoAtual, heuristica, posicaoAtual+1)
            posicaoAtual+=1
        if movimento == "e":
            caminhoTotal += movimentaPato(labirintoTeste, posicaoAtual, heuristica, posicaoAtual-1)
            posicaoAtual-=1
        if movimento == "b":
            caminhoTotal += movimentaPato(labirintoTeste, posicaoAtual, heuristica, posicaoAtual+10)
            posicaoAtual+=10
        if movimento == "c":
            caminhoTotal += movimentaPato(labirintoTeste, posicaoAtual, heuristica, posicaoAtual-10)
            posicaoAtual-=10
        if(posicaoAtual == 99):
            ...
            break
        heuristicaAtual = heuristica(posicaoAtual)
        labirinto[posicaoAtual] = heuristicaAtual + caminhoTotal
